...
function question2(data) {
  ...
  let oldData = []
  let round = 0
  while (newData.join('') !== oldData.join('')) {
    round++
    oldData = newData.slice()
    console.log({round})
    newData = newData.map((item, index, arr) => {
      
      if (item === '.') {
        return item
      }
      const isEmpty = item === "L"
      const isOccupied = item === "#"
      const prev = (index + 1) % rowLength !== 1 &&arr[index - 1]
      const next = (index + 1) % rowLength !== 0 && arr[index + 1]
      const top = arr[index - rowLength]
      const topPrev = (index + 1) % rowLength !== 1 && arr[index - rowLength - 1]
      const topNext = (index + 1) % rowLength !== 0 && arr[index - rowLength + 1]
      const bottom = arr[index + rowLength]
      const bottomPrev = (index + 1) % rowLength !== 1 && arr[index + rowLength - 1]
      const bottomNext = (index + 1) % rowLength !== 0 && arr[index + rowLength + 1]
      const numberOfNeighbors = [prev, next, top, topPrev, topNext, bottom, bottomPrev, bottomNext].reduce((tot, item) => {
        return tot + (item && item === '#' ? 1 : 0)
      }, 0)
      // console.log(item, prev, next, top, topPrev, topNext, bottom, bottomPrev, bottomNext, numberOfNeighbors)
      
      if (isEmpty && numberOfNeighbors === 0) {
        return '#'
      }
  
      if (isOccupied && numberOfNeighbors >= 4) {
        return 'L'
      }
    ...
    // console.log(newData.reduce((result, item, index) => {
    //   const isNewRow = index % rowLength === 0
    //   if (isNewRow) {
    //     result.push(item)
    //   } else {
  ...
  const numberOfOccupied = newData.reduce((tot, item) => tot + (item === '#' ? 1 : 0), 0)
  console.log('result1', numberOfOccupied)
}
...
function getPrev(arr, index, rowLength) {
  while ((index + 1) % rowLength !== 1 && arr[index - 1] === '.') {
    index--
  }
  return (index + 1) % rowLength !== 1 && arr[index - 1]
}

function getNext(arr, index, rowLength) {
  while ((index + 1) % rowLength !== 0 && arr[index + 1] === '.') {
    index++
  }
  return (index + 1) % rowLength !== 0 && arr[index + 1]
}

function getTop(arr, index, rowLength) {
  while (index > 0 && arr[index - rowLength] === '.') {
    index = index - rowLength
  }
  return arr[index - rowLength]
}

function getTopPrev(arr, index, rowLength) {
  let n = 1
  while ((index + 1) % rowLength !== 1 && arr[index - rowLength - n] === '.') {
    n--
    index = index - rowLength - n
  }
  return (index + 1) % rowLength !== 1 && arr[index - rowLength - n]
}

function getTopNext(arr, index, rowLength) {
  let n = 1
  while ((index + 1) % rowLength !== 0 && arr[index - rowLength + n] === '.') {
    n++
    index = index - rowLength + n
  }
  return (index + 1) % rowLength !== 0 && arr[index - rowLength + n]
}

function getBottom(arr, index, rowLength) {
  while (index < arr.length && arr[index + rowLength] === '.') {
    index = index + rowLength
  }
  return indexx < arr.length && arr[index + rowLength]
}

function getBottomPrev(arr, index, rowLength) {
  let n = 1
  while ((index + 1) % rowLength !== 1 && arr[index + rowLength - n] === '.') {
    n--
    index = index + rowLength - n
...
function getBottomNext(arr, index, rowLength) {
  let n = 1
  while ((index + 1) % rowLength !== 0 && arr[index + rowLength + n] === '.') {
    n++
    index = index + rowLength + n
...
function question2(data) {
  ...
  while (newData.join('') !== oldData.join('')) {
    ...
    newData = newData.map((item, index, arr) => {
      ...
      const isEmpty = item === "L"
      const isOccupied = item === "#"
      const prev = getPrev(arr, index, rowLength)
      const next = getNext(arr, index, rowLength)
      const top = getTop(arr, index, rowLength)
      const topPrev = getTopPrev(arr, index, rowLength)
      const topNext = getTopNext(arr, index, rowLength)
      const bottom = getBottom(arr, index, rowLength)
      const bottomPrev = getBottomPrev(arr, index, rowLength)
      const bottomNext = getBottomNext(arr, index, rowLength)
      const numberOfNeighbors = [prev, next, top, topPrev, topNext, bottom, bottomPrev, bottomNext].reduce((tot, item) => {
        return tot + (item && item === '#' ? 1 : 0)
      }, 0)
      // console.log(item, prev, next, top, topPrev, topNext, bottom, bottomPrev, bottomNext, numberOfNeighbors)
      ...
      if (isOccupied && numberOfNeighbors >= 5) {
        return 'L'
      }
  ...
  const numberOfOccupied = newData.reduce((tot, item) => tot + (item === '#' ? 1 : 0), 0)
  console.log('result2', numberOfOccupied)
}
