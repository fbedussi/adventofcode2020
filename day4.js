...
const validation = {
  ...
  hgt: (data) => {
    ...
    const [_, nStr, unit] = match;
    const n = parseInt(nStr);
    const isValidCm = unit === "cm" && n >= 150 && n <= 193;
    const isValidIn = unit === "in" && n >= 50 && n <= 76;
    console.log("hgt", n, unit, isValidCm, isValidIn);
    return isValidCm || isValidIn;
  },
  hcl: (data) => {
    return Boolean(data.match(/^#[0-9a-f]{6}$/));
  },
  ecl: (data) => {
    const validvalues = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"];
    return validvalues.includes(data);
  },
  pid: (data) => {
    return Boolean(data.match(/^\d{9}$/));
  },
};
...
const result = data.reduce((total, item) => {
  ...
  const isValid = mandatoryFields.every(
    (field) => fields.includes(field) && validation[field](item[field])
  );
  // console.log(item);
