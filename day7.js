...
const decode = (data) =>
  data.map((rule) => {
    ...
    if (!match) {
      throw new Error("no match1");
    }
    const [_, container, contentStr] = match;
    const content = contentStr.split(",").map((bagString) => {
      const match = bagString.trim().match(/^(no|\d) (.+) bags?/);
      if (!match) {
        throw new Error("no match2");
      }
      const [_, number, color] = match;
      return {
        number: number === "no" ? 0 : parseInt(number),
...
const decodedRules = decode(data);
const decodedRulesTest = decode(dataTest);

function findContainer(contentColor) {
  return decodedRules
    .filter(({ content }) =>
      content.find(({ color }) => color === contentColor)
    )
    .map(({ container }) => container)
    .flatMap((color) => [color, ...findContainer(color)]);
}

const result1 = new Set(findContainer("shiny gold"));
...
function countInnerBags(containerColor) {
  ...
  return decodedRules
    ...
    .reduce((tot, { content }) => {
      return content.reduce((tot2, { number, color }) => {
        return tot2 + number * (color ? countInnerBags(color) : 1);
      }, tot);
    }, 1);
}

const result2 = countInnerBags("shiny gold");
