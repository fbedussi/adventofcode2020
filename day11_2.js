...
function getNext(line, index) {
  while (index < line.length - 1 && line[index + 1] === ".") {
    index++;
  }
  return line[index + 1];
}

function getTop(lines, lineIndex, colIndex) {
  while (lineIndex > 0 && lines[lineIndex - 1][colIndex] === ".") {
    lineIndex = lineIndex - 1;
  }
  return lines[lineIndex - 1] && lines[lineIndex - 1][colIndex];
}

function getTopPrev(lines, lineIndex, colIndex) {
  while (
    lineIndex > 0 &&
    colIndex > 0 &&
    lines[lineIndex - 1][colIndex - 1] === "."
  ) {
    lineIndex = lineIndex - 1;
    colIndex = colIndex - 1;
  }
  return lines[lineIndex - 1] && lines[lineIndex - 1][colIndex - 1];
}

function getTopNext(lines, lineIndex, colIndex) {
  while (
    lineIndex > 0 &&
    colIndex < lines[0].length - 1 &&
    lines[lineIndex - 1][colIndex + 1] === "."
  ) {
    lineIndex = lineIndex - 1;
    colIndex = colIndex + 1;
...
function getBottom(lines, lineIndex, colIndex) {
  while (
    lineIndex < lines.length - 1 &&
    lines[lineIndex + 1][colIndex] === "."
  ) {
    lineIndex = lineIndex + 1;
  }
  return lines[lineIndex + 1] && lines[lineIndex + 1][colIndex];
...
function getBottomPrev(lines, lineIndex, colIndex) {
  while (
    lineIndex < lines.length - 1 &&
    colIndex > 0 &&
    lines[lineIndex + 1][colIndex - 1] === "."
...
function getBottomNext(lines, lineIndex, colIndex) {
  while (
    lineIndex < lines.length - 1 &&
    colIndex < lines[0].length - 1 &&
    lines[lineIndex + 1][colIndex + 1] === "."
...
function question2(data) {
  ...
  let newData = data.slice().map((line) => line.split(""));
  let oldData = [];
  let round = 0;
  while (newData.join("") !== oldData.join("")) {
    ...
    console.log({ round });
    newData = newData.map((line, lineIndex, lines) => {
      return line.map((col, colIndex, line) => {
        if (col === ".") {
          return col;
        }
        ...
        const prev = getPrev(line, colIndex);
        const next = getNext(line, colIndex);
        const top = getTop(lines, lineIndex, colIndex);
        const topPrev = getTopPrev(lines, lineIndex, colIndex);
        const topNext = getTopNext(lines, lineIndex, colIndex);
        const bottom = getBottom(lines, lineIndex, colIndex);
        const bottomPrev = getBottomPrev(lines, lineIndex, colIndex);
        const bottomNext = getBottomNext(lines, lineIndex, colIndex);
  ...
  const numberOfOccupied = newData
    .flatMap((line) => line)
    .reduce((tot, item) => tot + (item === "#" ? 1 : 0), 0);
